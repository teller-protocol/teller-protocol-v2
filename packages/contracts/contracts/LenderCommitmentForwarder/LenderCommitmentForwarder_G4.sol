...
import "../interfaces/ILenderCommitmentForwarder_G4.sol";
...
contract LenderCommitmentForwarder_G4 is
    ...
    ILenderCommitmentForwarder_G4
{
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;

    // CommitmentId => commitment
    mapping(uint256 => Commitment) public commitments;
    mapping(uint256 => CommitmentRestrictions ) public commitmentRestrictions;
    ...
    function createCommitmentWithRestrictions(
        Commitment calldata _commitment,
        CommitmentRestrictions calldata _restrictions,
        address[] calldata _borrowerAddressList
    ) public returns (uint256 commitmentId_) {
        ...
        commitments[commitmentId_] = _commitment;
        commitmentRestrictions[commitmentId_] = _restrictions;
    ...
    function acceptCommitmentForRollover(
            uint256 _commitmentId,
            uint256 _rolloverFromBidId,
            uint256 _principalAmount,
            uint256 _collateralAmount,
            uint256 _collateralTokenId,
            address _collateralTokenAddress,
...
*/
    ...
    function _acceptCommitment(
        ...
        CommitmentRestrictions storage restrictions = commitmentRestrictions[_commitmentId];

        //make sure the commitment data adheres to required specifications and limits
        validateCommitment(commitment);

        require(
            restrictions.acceptedByAddress == address(0) || 
            restrictions.acceptedByAddress == msg.sender ,
            "Commitment Restriction: Invalid AcceptedByAddress"
        );

        require(
            restrictions.rolloverFromBidId == 0 || 
            restrictions.rolloverFromBidId == _rolloverFromBidId ,
            "Commitment Restriction: Invalid RolloverFromBidId"
        );
