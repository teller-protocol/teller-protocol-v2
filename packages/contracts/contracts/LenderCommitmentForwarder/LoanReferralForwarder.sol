...
contract LoanReferralForwarder {
    ...
    function acceptCommitmentWithReferral(
        ...
        require(
            _reward <= _acceptCommitmentArgs.principalAmount / 10,
            "Reward can be no more than 10% of principal"
        );

        if (_acceptCommitmentArgs.smartCommitmentAddress != address(0)) {
            principalTokenAddress = ISmartCommitment(
                _acceptCommitmentArgs.smartCommitmentAddress
            ).getPrincipalTokenAddress();

            // Accept commitment and receive funds to this contract
            balanceBefore = IERC20(principalTokenAddress).balanceOf(
                address(this)
            );
            ...
            principalTokenAddress = ILenderCommitmentForwarder_U1(
                _commitmentForwarder
        ...
        uint256 balanceAfter = IERC20(principalTokenAddress).balanceOf(
            address(this)
        );
    ...
    function _acceptCommitmentWithRecipient(
        ...
        bytes memory responseData = address(_commitmentForwarder).functionCall(
            abi.encodePacked(
                abi.encodeWithSelector(
