...
const deployFn: DeployFunction = async (hre) => {
  hre.log('----------')
  hre.log('')
  hre.log('Robinhood: Transferring ownership of contracts to Safe Multisig')
  hre.log('')

  const { deployer, protocolOwnerSafe } = await hre.getNamedAccounts()

  if (protocolOwnerSafe === '0x0000000000000000000000000000000000000000') {
    hre.log(
      '  ⚠️  protocolOwnerSafe is the zero address — set ROBINHOOD_SAFE_ADDRESS and re-run. Skipping.'
    )
    return true
  }
...
// tags and deployment
deployFn.id = 'robinhood:transfer-ownership-to-safe'
deployFn.tags = ['robinhood', 'robinhood:transfer-ownership-to-safe']
...
deployFn.skip = async (hre) => {
  return !hre.network.live || hre.network.name !== 'robinhood'
}
