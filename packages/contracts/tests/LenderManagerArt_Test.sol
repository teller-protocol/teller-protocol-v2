...
contract LenderManagerArt_Test is Testable {
    ...
    function test_get_token_decimals() public {
        uint256 decimals = LenderManagerArt._get_token_decimals(
            address(wethMock)
        );
    ...
    function test_get_interest_rate_formatted() public {
        string memory interestRate = LenderManagerArt
            ._get_interest_rate_formatted(30000);

        assertEq(interestRate, "300 %");
    }

    function test_get_duration_formatted() public {
        string memory duration = LenderManagerArt._get_duration_formatted(
            30000
        );
    ...
    function test_get_token_symbol() public {
        string memory symbol = LenderManagerArt._get_token_symbol(
            address(wethMock),
            "fallback"
        );
    ...
    function test_get_collateral_label() public {
        ...
        string memory label = LenderManagerArt._get_collateral_label(
            _collateral
        );
