...
function deployIntegrationSuite() public returns (address tellerV2_){
    ...
    TellerV2 tellerV2 = new TellerV2(trustedForwarder);
        ...
        LenderCommitmentForwarder _lenderCommitmentForwarder = new LenderCommitmentForwarder(address(tellerV2), address(_marketRegistry));
        CollateralManager _collateralManager = new CollateralManager();
        LenderManager _lenderManager = new LenderManager(IMarketRegistry(_marketRegistry));
        EscrowVault _escrowVault = new EscrowVault();

        CollateralEscrowV1 collateralEscrowBeacon = new CollateralEscrowV1();
    _collateralManager.initialize(address(collateralEscrowBeacon),address(tellerV2));
