...
POLYGON 
const FACTORY_TRACKED_CONTRACT: [u8; 20] = hex!("0x08d8d9c157d4c038e6b50cc8514ecd31b66b3157");
const COLLATERAL_MANAGER_TRACKED_CONTRACT: [u8;20] = hex!("76888a882a4fF57455B5e74B791DD19DF3ba51Bb");
 

ARBITRUM 
const FACTORY_TRACKED_CONTRACT: [u8; 20] = hex!(" ");
...
const FACTORY_TRACKED_CONTRACT: [u8; 20] = hex!("08d8d9c157d4c038e6b50cc8514ecd31b66b3157");
const COLLATERAL_MANAGER_TRACKED_CONTRACT: [u8;20] = hex!("76888a882a4fF57455B5e74B791DD19DF3ba51Bb");
...
fn graph_lendergroup_out(
    ...
    events.lendergroup_withdraws.iter().for_each(|evt| {
        tables
            .create_row("group_lender_withdraw", format!("{}-{}", evt.evt_tx_hash, evt.evt_index))
            .set("evt_tx_hash", evt.evt_tx_hash.clone().into_bytes())
            .set("evt_index", BigInt::from(evt.evt_index))
            .set("evt_block_time", BigInt::from(evt.evt_block_time))
            .set("evt_block_number", BigInt::from(evt.evt_block_number))
    ...
    events.lendergroup_deposits.iter().for_each(|evt| {
        tables
            .create_row("group_lender_deposit", format!("{}-{}", evt.evt_tx_hash, evt.evt_index))
            .set("evt_tx_hash", evt.evt_tx_hash.clone().into_bytes())
            .set("evt_index", BigInt::from(evt.evt_index))
            .set("evt_block_time", BigInt::from(evt.evt_block_time))
            .set("evt_block_number", BigInt::from(evt.evt_block_number))
        ...
        tables
          ...
          // let lender_group_contract_address = Hex::decode(&evt.evt_address).unwrap();
           ...
           let fetched_token_amount_difference = rpc::fetch_token_amount_difference_from_liquidations(&evt.evt_address).unwrap_or_default();
             ...
             let lender_group_contract_address = &evt.evt_address  ;
           //ok to do this here ?
...
fn store_lendergroup_user_metrics_deltas(
    ...
    events.lendergroup_deposits.iter().for_each(|evt: &contract::LendergroupDeposit| {
        ...
        let user_store_key: String = format!("group_user_metric:{}:{}:interaction_count", evt.evt_address,Hex(&evt.owner ).to_string());
        bigint_add_store.add(ord,&user_store_key, BigInt::from( 1 ));


        
        let user_store_key: String = format!("group_user_metric:{}:{}:total_principal_tokens_committed", evt.evt_address,Hex(&evt.owner).to_string());
        bigint_add_store.add(ord,&user_store_key, BigInt::from_str(&evt.assets ).unwrap_or(BigInt::zero()));
    ...
    events.lendergroup_withdraws.iter().for_each(|evt: &contract::LendergroupWithdraw | {
        ...
        let user_store_key: String = format!("group_user_metric:{}:{}:total_principal_tokens_withdrawn", evt.evt_address,Hex(&evt.owner).to_string());
        bigint_add_store.add(ord,&user_store_key, BigInt::from_str(&evt.assets).unwrap_or(BigInt::zero()));
...
fn store_lendergroup_pool_metrics_deltas(
    ...
    events.lendergroup_deposits.iter().for_each(|evt: &contract::LendergroupDeposit | {
        ...
        bigint_add_store.add(ord,&group_store_key, BigInt::from_str(&evt.assets).unwrap_or(BigInt::zero()));
